...
<section class="analisis">
  <div class="container">
    ...
    <br>
        <div class="row">
            <form method="post" action="hitung_sensitifitas.php">
...
<section class="analisis" id="pilihan">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= number_format($alternatif['nilai_saw'],3); ?></td>
                        <td><?= number_format($alternatif['nilai_saw_sen'],3); ?></td>
                        <td><?= number_format($alternatif['nilai_topsis'],3); ?></td>
                        <td><?= number_format($alternatif['nilai_topsis_sen'],3); ?></td>
                        <td><?= number_format($alternatif['nilai_saw_topsis'],3); ?></td>
                        <td><?= number_format($alternatif['nilai_saw_topsis_sen'],3); ?></td>
      ...
      <?php 
        ...
        if($totalPerubahan['perulangan'] != 0){
          ...
          <p>Perubahan SAW <?php echo number_format($diffsaw,3) ?>%</p>
          <p>Perubahan TOPSIS <?php echo number_format($difftopsis,3) ?>%</p>
          <p>Perubahan SAW-TOPSIS <?php echo number_format($diffsawtopsis,3) ?>%</p>


        <?php
          $sqlRekomendasi = "SELECT * FROM `alternatif` a JOIN `analisis_sensitifitas` h ON a.`id_alternatif` = h.`id_alternatif` ORDER BY h.`nilai_saw` DESC LIMIT 3";
          if(($difftopsis>$diffsaw) && ($difftopsis>$diffsawtopsis)){
            $sqlRekomendasi = "SELECT * FROM `alternatif` a JOIN `analisis_sensitifitas` h ON a.`id_alternatif` = h.`id_alternatif` ORDER BY h.`nilai_topsis` DESC LIMIT 3";
          } else if (($diffsawtopsis>$diffsaw) && ($diffsawtopsis>$difftopsis)) {
          	$sqlRekomendasi = "SELECT * FROM `alternatif` a JOIN `analisis_sensitifitas` h ON a.`id_alternatif` = h.`id_alternatif` ORDER BY h.`nilai_saw_topsis` DESC LIMIT 3";
          }
        ...
        <table class="table table-bordered" id="">
            ...
            <tr>
			      <th scope="col">No</th>
			      <th scope="col">Rekomendasi Prodi Terbaik</th>
